# studyplan-pipeline/harvestReferences/__init__.py - UPDATED FOR CATEGORY SUPPORT
...
def _fetch_index_references(topic_name: str, sub_title: str, category: str = None) -> list[str]:
    """
    Return unique bibliography reference strings from Azure Search index.
    
    NEW: Uses sub_subtopic field to match sub_title when category is provided.
    When category is None, matches against subtopic field (top-level items).
    """
    select = ["references", "topic", "subtopic", "sub_subtopic"]
    
    # Build filter based on whether this is a categorized subtopic or not
    if category:
        # This is a sub-subtopic: match against sub_subtopic field
        # Also match against parent subtopic (category) for broader coverage
        filt = (f"topic eq '{_escape_odata(topic_name)}' and "
                f"(sub_subtopic eq '{_escape_odata(sub_title)}' or "
                f"(subtopic eq '{_escape_odata(category)}' and sub_subtopic eq '{_escape_odata(sub_title)}'))")
    else:
        # Top-level subtopic: match against subtopic field where sub_subtopic is empty/null
        filt = (f"topic eq '{_escape_odata(topic_name)}' and "
                f"subtopic eq '{_escape_odata(sub_title)}'")
    
    docs = _search_all(search_text="*", filter=filt, select=select, top=1000)
    
    # Fallback: if no exact match, try broader search
    if not docs:
        if category:
            # Try just the category
            filt2 = f"topic eq '{_escape_odata(topic_name)}' and subtopic eq '{_escape_odata(category)}'"
        else:
            # Try topic + search for subtopic
            filt2 = f"subtopic eq '{_escape_odata(sub_title)}'"
        
        docs2 = _search_all(search_text=topic_name, filter=filt2, select=select, top=200)
        
        if docs2:
            # Find most common topic match
            freq: dict[str, int] = {}
            for d in docs2:
            ...
            if freq:
                ...
                if category:
                    filt3 = (f"topic eq '{_escape_odata(best)}' and "
...
def main(msg: func.QueueMessage) -> None:
    logging.info("harvestReferences triggered (CATEGORY-AWARE)")
    ...
    # Look up subtopic with category
    with pyodbc.connect(DB_CONN) as cx:
        cur = cx.cursor()
        cur.execute("""
            SELECT s.title, s.topic_id, t.topic_name, s.category
        ...
        if not row:
            ...
            return
        sub_title, topic_id, topic_name, category = row.title, row.topic_id, row.topic_name, row.category
    
    # Fetch references from index (category-aware)
    ref_strings = _fetch_index_references(topic_name, sub_title, category)
        ...
        with pyodbc.connect(DB_CONN) as cx:
            ...
            cur.execute("""
                ...
                SET status = 'refs_missing',
                    coverage_note = COALESCE(coverage_note, 'No bibliography references found in index')
                WHERE subtopic_id = ?
            """, subtopic_id)
            cx.commit()
        
        log_msg = f"No references found for topic='{topic_name}' subtopic='{sub_title}'"
        if category:
            log_msg += f" category='{category}'"
        logging.warning(log_msg + " → refs_missing")
        return
    ...
    logging.info("Found %d bibliography reference(s) for '%s'%s", 
                 len(refs), sub_title, f" (category: {category})" if category else "")
