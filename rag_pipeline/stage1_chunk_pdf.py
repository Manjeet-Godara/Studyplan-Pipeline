...
def is_heading(line: str) -> bool:
    ...
    Since all AI-generated headings contain ';' and are uppercase,
    we just need to check these two conditions.
    """
    ln = line.strip()
...
def parse_heading(heading: str) -> Tuple[str, str, Optional[str]]:
    ...
    Enhanced to handle optional trailing delimiter.
    ...
    parts = [p.strip() for p in heading_clean.split(HEADING_SEPARATOR)]
...
def merge_split_headings(lines: List[str]) -> List[str]:
    ...
    Simple strategy since all headings are guaranteed to end with ';':
    1. If a line looks like a heading but doesn't end with ';', it's incomplete
    2. Keep merging subsequent heading-like lines until we find the ';'
    3. This captures the complete title regardless of PDF line breaks
    """
    merged = []
    i = 0
    ...
    while i < len(lines):
        ...
        if is_heading(line):
            # Start accumulating the heading
            accumulated_heading = line
            
            # If it doesn't end with ';', it's split across lines
            # Keep merging until we find the delimiter
            j = i + 1
            while not accumulated_heading.rstrip().endswith(';') and j < len(lines):
                ...
                # Check if next line continues the heading
                if is_heading(next_line):
                    # Merge with space
                    accumulated_heading += " " + next_line
                    j += 1
                else:
                    # Hit content without finding ';' - should not happen with AI-generated docs
                    # but handle gracefully
                    break
            
            merged.append(accumulated_heading.strip())
...
def chunk_to_pdf(chunk: ChunkMetadata, out_path: Path) -> None:
    ...
    # Title (heading path) - wrap if needed
    c.setFont("Helvetica-Bold", 12)
    title_lines = wrap(chunk.heading_path, 80)
    for ln in title_lines[:3]:  # Max 3 lines for title
        c.drawString(40, y, ln)
        y -= 16
    
    # Metadata - FIXED: Display complete hierarchy without truncation
    y -= 10
    c.setFont("Helvetica-Oblique", 9)
    
    # Build metadata lines (wrap if needed)
    metadata_parts = [
        f"Topic: {chunk.topic}",
    ...
    if chunk.sub_subtopic:
        metadata_parts.append(f"Sub-subtopic: {chunk.sub_subtopic}")
    
    if chunk.total_chunks > 1:
        metadata_parts.append(f"Part {chunk.chunk_index + 1}/{chunk.total_chunks}")
    
    # Join with separator and wrap to multiple lines if needed
    meta_text = " | ".join(metadata_parts)
    meta_lines = wrap(meta_text, 110)  # 110 chars per line fits nicely
    
    for meta_line in meta_lines[:3]:  # Max 3 lines for metadata
        c.drawString(40, y, meta_line)
        y -= 12
    
    y -= 10  # Extra spacing before content
