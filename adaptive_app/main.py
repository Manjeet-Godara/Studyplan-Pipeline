...
def api_answer(ans: AnswerIn, db: Session = Depends(get_db)):
    ...
    """
    sess: Session | None = db.query(Session).get(ans.session_id)
    if not sess:
        raise HTTPException(status_code=404, detail="Session not found")
    if str(sess.topic_id).lower() != str(ans.topic_id).lower():
        raise HTTPException(status_code=400, detail="Answer topic does not match session topic")
    if str(sess.status or "").lower() == "terminated":
        raise HTTPException(status_code=409, detail="Terminated sessions cannot accept answers")
    ...
    # Completed sessions can receive free targeted reattempt answers. They must
    # never change the persisted summary, so we only ensure a temporary live bucket.
    store.ensure(ans.session_id, sess.user_id, sess.topic_id)
    record_attempt(
        db,
...
QUICK_REASSESSMENT_START = "<!-- AICME_QUICK_REASSESSMENT_START -->"
QUICK_REASSESSMENT_END = "<!-- AICME_QUICK_REASSESSMENT_END -->"
REATTEMPTED_START = "<!-- AICME_REATTEMPTED_SUBTOPICS_START -->"
REATTEMPTED_END = "<!-- AICME_REATTEMPTED_SUBTOPICS_END -->"
REATTEMPT_LINK_PREFIX = "#reattempt-subtopic-"


def _normalize_subtopic_title(title: Any) -> str:
    return re.sub(r"\s+", " ", str(title or "").strip()).casefold()


def _markdown_link_text(text: Any) -> str:
    return str(text or "").replace("\\", "\\\\").replace("[", "\\[").replace("]", "\\]")


def _subtopic_rows_for_topic(topic_id: UUID) -> list[dict]:
    plan = load_plan(topic_id)
    return list((plan or {}).get("subtopics") or [])


def _subtopic_by_id(topic_id: UUID, subtopic_id: UUID) -> dict | None:
    wanted = str(subtopic_id).lower()
    for subtopic in _subtopic_rows_for_topic(topic_id):
        if str(subtopic.get("subtopic_id", "")).lower() == wanted:
            return subtopic
    return None


def _subtopic_by_title(topic_id: UUID) -> dict[str, dict]:
    out: dict[str, dict] = {}
    for subtopic in _subtopic_rows_for_topic(topic_id):
        title = subtopic.get("subtopic_title")
        if title:
            out[_normalize_subtopic_title(title)] = subtopic
    return out


def _load_per_subtopic(summary: SessionSummary | None) -> dict:
    if not summary or not summary.per_subtopic_json:
        return {}
    try:
        parsed = json.loads(summary.per_subtopic_json)
    except Exception:
        return {}
    return parsed if isinstance(parsed, dict) else {}


def _extract_managed_block(markdown: str, start: str, end: str) -> str:
    pattern = re.compile(re.escape(start) + r"(.*?)" + re.escape(end), re.DOTALL)
    match = pattern.search(markdown or "")
    return match.group(1).strip() if match else ""


def _without_managed_block(markdown: str, start: str, end: str) -> str:
    pattern = re.compile(r"\n*\s*" + re.escape(start) + r".*?" + re.escape(end) + r"\s*", re.DOTALL)
    return pattern.sub("\n\n", markdown or "").strip()


def _extract_reattempted_titles(markdown: str) -> list[str]:
    block = _extract_managed_block(markdown, REATTEMPTED_START, REATTEMPTED_END)
    titles: list[str] = []
    for line in block.splitlines():
        ...
        if not title or title.lower() in {"none yet.", "none yet"}:
            continue
        titles.append(title)
    return titles


def _merge_reattempted_titles(markdown: str, new_titles: list[str] | None = None) -> list[str]:
    merged: list[str] = []
    seen: set[str] = set()
    for title in [*_extract_reattempted_titles(markdown), *(new_titles or [])]:
        cleaned = " ".join(str(title or "").split())
        key = _normalize_subtopic_title(cleaned)
...
def _weak_subtopic_rows(topic_id: UUID, per_subtopic: dict) -> list[dict]:
    by_title = _subtopic_by_title(topic_id)
    rows: list[dict] = []
    for title, raw in (per_subtopic or {}).items():
        rec = raw or {}
        ...
        if correct >= total:
            continue
        subtopic = by_title.get(_normalize_subtopic_title(title))
        if not subtopic:
            continue
        accuracy = round((correct / total) * 100.0, 1)
        rows.append({
            "title": subtopic.get("subtopic_title") or title,
            "subtopic_id": str(subtopic.get("subtopic_id")),
            "correct": correct,
            "total": total,
            "accuracy": accuracy,
        })
    return sorted(rows, key=lambda row: (row["accuracy"], row["title"]))[:3]


def _with_report_action_sections(
    markdown: str,
    topic_id: UUID,
    per_subtopic: dict,
    *,
    reattempted_titles: list[str] | None = None,
) -> str:
    base = _without_managed_block(markdown or "", QUICK_REASSESSMENT_START, QUICK_REASSESSMENT_END)
    base = _without_managed_block(base, REATTEMPTED_START, REATTEMPTED_END)
    reattempted = _merge_reattempted_titles(markdown or "", reattempted_titles)
    weak_rows = _weak_subtopic_rows(topic_id, per_subtopic)

    quick_lines = [
        QUICK_REASSESSMENT_START,
        "## Quick free assessment for weak sub-topics",
    ]
    if weak_rows:
        quick_lines.append("Click a weak sub-topic below to take a short assessment focused only on that sub-topic. This reattempt is free of cost and does not change the performance numbers above.")
        quick_lines.append("")
        for row in weak_rows:
            quick_lines.append(
                f"- [{_markdown_link_text(row['title'])}]({REATTEMPT_LINK_PREFIX}{row['subtopic_id']}) - {row['correct']} / {row['total']} correct ({row['accuracy']}%)"
            )
    else:
        quick_lines.append("No weak sub-topics were identified from this attempt.")
    quick_lines.append(QUICK_REASSESSMENT_END)

    reattempted_lines = [
        REATTEMPTED_START,
        "## Reattempted subtopics",
    ]
    if reattempted:
        reattempted_lines.extend(f"- {title}" for title in reattempted)
    else:
        reattempted_lines.append("- None yet.")
    reattempted_lines.append(REATTEMPTED_END)

    return "\n\n".join(part for part in [
        base,
        "\n".join(quick_lines),
        "\n".join(reattempted_lines),
    ] if part).strip()


def _ensure_report_action_sections(summary: SessionSummary) -> bool:
    per_subtopic = _load_per_subtopic(summary)
    updated = _with_report_action_sections(
        summary.report_markdown or "",
        summary.topic_id,
        per_subtopic,
    )
    if updated != (summary.report_markdown or ""):
        summary.report_markdown = updated
        return True
    return False
...
def api_report(session_id: UUID, db: Session = Depends(get_db)):
    ...
    if existing and existing.report_markdown:
        if _ensure_report_action_sections(existing):
            db.commit()
            db.refresh(existing)
        return ReportOut(
    ...
    md = _with_report_action_sections(run_gpt_report(ctx), s.topic_id, per_sub)
...
@app.post("/api/report/{session_id}/reattempted-subtopics", response_model=ReportOut)
def api_report_mark_reattempted_subtopic(
    session_id: UUID,
    subtopic_id: UUID = Body(..., embed=True),
    db: Session = Depends(get_db),
):
    sess: Session | None = db.query(Session).get(session_id)
    if not sess:
    ...
    if not summary or not summary.report_markdown:
        raise HTTPException(status_code=404, detail="Generated report not found")

    subtopic = _subtopic_by_id(sess.topic_id, subtopic_id)
    if not subtopic:
        raise HTTPException(status_code=404, detail="Subtopic not found for this session topic")

    subtopic_title = subtopic.get("subtopic_title") or str(subtopic_id)
    per_subtopic = _load_per_subtopic(summary)
    summary.report_markdown = _with_report_action_sections(
        summary.report_markdown,
        sess.topic_id,
        per_subtopic,
        reattempted_titles=[subtopic_title],
    )
    db.commit()
    store.pop(session_id)
    return ReportOut(
