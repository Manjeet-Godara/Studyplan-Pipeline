...
const REATTEMPT_LINK_PREFIX = "#reattempt-subtopic-";
const HOME_ACTION_WIDTH = 180;
const HOME_ACTION_CONTROL_SX = {
  borderRadius: 25,
  height: 44,
  width: { xs: "100%", sm: HOME_ACTION_WIDTH },
  minWidth: { xs: "100%", sm: HOME_ACTION_WIDTH },
  maxWidth: { xs: "100%", sm: HOME_ACTION_WIDTH },
  boxSizing: "border-box",
  fontSize: "0.875rem",
  fontWeight: "bold",
  whiteSpace: "nowrap",
};
const HOME_ACTION_BUTTON_SX = {
  ...HOME_ACTION_CONTROL_SX,
  px: 1.5,
  py: 0,
  textTransform: "none",
...
const HOME_ACTION_GROUP_SX = {
  ...
  width: {
    xs: "100%",
    md: HOME_ACTION_WIDTH * 2 + 12,
    xl: HOME_ACTION_WIDTH * 4 + 36,
  },
  maxWidth: "100%",
};
...
export default function AdaptiveApp() {
  ...
  const [quickAssessmentPrompt, setQuickAssessmentPrompt] = useState({ open: false, subtopic: null });
  const [completedQuickAssessmentTitle, setCompletedQuickAssessmentTitle] = useState("");
  const skipNextReportLoadRef = useRef(false);
  ...
  const resetQuickAssessmentState = () => {
    setQuickAssessment(null);
    setQuickAssessmentPrompt({ open: false, subtopic: null });
    setCompletedQuickAssessmentTitle("");
    skipNextReportLoadRef.current = false;
  };
  ...
  const saveSessionSnapshot = async (overrides = {}) => {
    if (!sessionId || !topicId || !plan || finished || quickAssessment) return null;
  ...
  useEffect(() => {
    if (!sessionId || !topicId || !plan || finished || quickAssessment) return;
  ...
  }, [sessionId, topicId, plan, finished, quickAssessment, subIdx, mcqIdx, attemptIdx, view, tab, history]);
  ...
  }, [sessionId, topicId, plan, finished, quickAssessment, subIdx, mcqIdx, attemptIdx, view, tab, history, sessionElapsedSeconds]);

  /* ---------- report generation on finish ---------- */
  useEffect(() => {
    if (!finished || !sessionId) return;
    if (skipNextReportLoadRef.current) {
      skipNextReportLoadRef.current = false;
      return;
    }
    let cancelled = false;
  ...
  const questionCount = (subtopic) =>
    Array.isArray(subtopic?.questions) ? subtopic.questions.length : 0;
  const firstQuestionCursor = (planJson = plan) => {
    const subtopics = planJson?.subtopics || [];
    for (let i = 0; i < subtopics.length; i++) {
      if (questionCount(subtopics[i]) > 0) return { subIdx: i, mcqIdx: 0 };
    }
    return null;
  };
  const nextQuestionCursor = (fromSubIdx = subIdx, fromMcqIdx = mcqIdx, planJson = plan) => {
    const subtopics = planJson?.subtopics || [];
    const currentCount = questionCount(subtopics[fromSubIdx]);
    if (currentCount > 0 && fromMcqIdx < currentCount - 1) {
      return { subIdx: fromSubIdx, mcqIdx: fromMcqIdx + 1 };
    }
    for (let i = fromSubIdx + 1; i < subtopics.length; i++) {
      if (questionCount(subtopics[i]) > 0) return { subIdx: i, mcqIdx: 0 };
    }
    return null;
  };
  ...
  const getProgress = () => {
    if (!plan) return 0;
    if (quickAssessment && currentSub()) {
      const totalQuestions = questionCount(currentSub()) || 1;
      return ((mcqIdx + 1) / totalQuestions) * 100;
    }
    const subtopics = plan.subtopics || [];
    const totalQuestions = subtopics.reduce((sum, sub) => sum + questionCount(sub), 0);
    if (totalQuestions === 0) return 0;
    const questionsBefore = subtopics.slice(0, subIdx).reduce((sum, sub) => sum + questionCount(sub), 0);
    const currentQuestionNumber = Math.min(questionsBefore + mcqIdx + 1, totalQuestions);
    return (currentQuestionNumber / totalQuestions) * 100;
  };

  const findSubtopicById = (subtopicId) =>
    (plan?.subtopics || []).find(st => normalizeId(st.subtopic_id) === normalizeId(subtopicId));

  const subtopicIdFromReattemptHref = (href) => {
    const raw = String(href || "");
    const markerIndex = raw.indexOf(REATTEMPT_LINK_PREFIX);
    if (markerIndex === -1) return "";
    return raw.slice(markerIndex + REATTEMPT_LINK_PREFIX.length).split(/[?#&]/)[0];
  };
  ...
  const handleReportClick = (event) => {
    ...
    const subtopicId = subtopicIdFromReattemptHref(anchor.getAttribute("href"));
    if (!subtopicId) return;

    event.preventDefault();
    const subtopic = findSubtopicById(subtopicId);
    if (!subtopic || !sessionId) {
      alert("This quick assessment is not available from the current report view.");
      return;
    }
    setQuickAssessmentPrompt({ open: true, subtopic });
  };
  ...
  const beginQuestions = async () => {
    try {
      resetQuickAssessmentState();
      const firstQuestion = firstQuestionCursor();
      if (!firstQuestion) {
        alert("This study plan does not have any questions available yet.");
        return;
      }
      if (!hasActiveSession) {
        setLoading(true);
        ...
        setHistory([]);
        setSubIdx(firstQuestion.subIdx);
        setMcqIdx(firstQuestion.mcqIdx);
        setAttemptIdx(0);
        setChoice(null);
        setResult(null);
  ...
  const beginQuickAssessment = (subtopic) => {
    const targetIndex = (plan?.subtopics || []).findIndex(
      st => normalizeId(st.subtopic_id) === normalizeId(subtopic?.subtopic_id)
    );
    if (targetIndex < 0 || !sessionId || questionCount(plan?.subtopics?.[targetIndex]) === 0) {
      alert("Unable to start this quick assessment from the current report.");
      return;
    }
  ...
  const completeQuickAssessment = async () => {
    const completedTitle = quickAssessment?.title || currentSub()?.subtopic_title || "";
    pauseSessionTimer();
    clearAutoNextTimers();
    setLoadingReport(false);

    try {
      const { data } = await axios.post(`/api/api/report/${sessionId}/reattempted-subtopics`, {
        subtopic_id: quickAssessment?.subtopicId || currentSub()?.subtopic_id,
      });
      ...
      console.error("Error updating reattempted subtopics:", error);
      alert("Your quick assessment is complete, but the report could not be updated. Please reopen the report and try again.");
    }

    skipNextReportLoadRef.current = true;
    setCompletedQuickAssessmentTitle(completedTitle);
    setQuickAssessment(null);
    setChoice(null);
    setResult(null);
    setMode("question");
  ...
  const proceed = async () => {
    ...
    setAutoNextIn(0);
    if (quickAssessment) {
      const lastQuickMcq = questionCount(currentSub()) - 1;
      if (mcqIdx >= lastQuickMcq) {
        await completeQuickAssessment();
        return;
      }
      setMcqIdx(mcqIdx + 1);
  ...
  const renderBuyCreditsButton = (sx = {}) => {
  ...
  const quickAssessmentDialog = (
    <Dialog
      open={quickAssessmentPrompt.open}
      onClose={() => setQuickAssessmentPrompt({ open: false, subtopic: null })}
      fullWidth
      maxWidth="sm"
    >
      <DialogTitle sx={{ fontWeight: 'bold', color: '#2c3e50' }}>
        Start Free Quick Assessment?
      </DialogTitle>
      <DialogContent>
        <Stack spacing={2}>
          <Typography sx={{ color: '#34495e', lineHeight: 1.7 }}>
            This will start a short assessment for only this subtopic:
          </Typography>
          <Alert severity="info" sx={{ borderRadius: 2 }}>
            <Typography sx={{ fontWeight: 'bold' }}>
              {quickAssessmentPrompt.subtopic?.subtopic_title}
            </Typography>
            <Typography variant="body2">
              Free of cost. Your original performance numbers will stay unchanged.
            </Typography>
          </Alert>
        </Stack>
      </DialogContent>
      <DialogActions sx={{ px: 3, pb: 3 }}>
        <Button
          onClick={() => setQuickAssessmentPrompt({ open: false, subtopic: null })}
          variant="outlined"
          sx={{
            borderRadius: 2,
            textTransform: 'none',
        ...
        <Button
          onClick={() => beginQuickAssessment(quickAssessmentPrompt.subtopic)}
          variant="contained"
          sx={{
            borderRadius: 2,
            textTransform: 'none',
                ...
                <Box sx={{
                  ...
                  <Paper sx={{
                    ...
                    <Breadcrumbs sx={{ '& .MuiBreadcrumbs-separator': { color: 'white' } }}>
                      <Link
                        underline="hover"
                        onClick={quickAssessment ? undefined : goHome}
                        sx={{
                          cursor: quickAssessment ? "default" : "pointer",
                          color: 'white',
                          display: 'flex',
                          alignItems: 'center',
                          gap: 1,
                      ...
                      <Link
                        underline="hover"
                        onClick={quickAssessment ? undefined : () => setView("topics")}
                        sx={{
                          cursor: quickAssessment ? "default" : "pointer",
                          color: 'white',
                          '&:hover': { opacity: 0.8 }
                      ...
                      <Link
                        underline="hover"
                        onClick={quickAssessment ? undefined : openCurrentCategory}
                        sx={{
                          cursor: quickAssessment ? "default" : "pointer",
                          color: 'white',
                          '&:hover': { opacity: 0.8 }
                        ...
                        <Paper sx={{
                          ...
                          {!currentQ() && (
                            <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
                              This section has no questions available. Continue to the next question.
                            </Alert>
                          )}
                          ...
                          <Stack direction="row" justifyContent="center" sx={{ mt: 3 }}>
                            ...
                            <Button
                              variant="contained"
                              disabled={!currentQ() || choice === null || awaitingAutoNext}
                              onClick={submit}
                              sx={{
                                borderRadius: 3,
                                px: 4,
                    ...
                    <Slide direction="up" in timeout={500}>
                      <Box>
                        ...
                        <Stack direction="row" justifyContent="center">
                          ...
                          >
                            {quickAssessment && mcqIdx >= questionCount(currentSub()) - 1
                              ? "Finish Quick Assessment"
                              : "Continue Learning"}
                          </Button>
                        </Stack>
                      </Box>
              ...
              <Paper sx={{
                ...
                <Box sx={{
                  ...
                  <Typography variant="h4" sx={{ fontWeight: 'bold', mb: 1 }}>
                    {completedQuickAssessmentTitle ? "Quick Assessment Complete" : "Congratulations!"}
                  </Typography>
                  <Typography variant="h6" sx={{ opacity: 0.9 }}>
                    {completedQuickAssessmentTitle
                      ? `You reattempted ${completedQuickAssessmentTitle}`
                  ...
                  {loadingReport && (
                    <Stack direction="row" spacing={3} justifyContent="center" sx={{ py: 4 }}>
                      ...
                      <Typography variant="h6" sx={{ color: '#ff6b35' }}>
                        {reportMd ? "Loading your saved report..." : "Generating your personalized report..."}
                      </Typography>
                    </Stack>
                  )}
	                    ...
	                    <Fade in timeout={600}>
	                            ...
	                            >
	                              <Box>
	                                ...
	                                <Typography sx={{ color: '#34495e', lineHeight: 1.6 }}>
	                                  {completedQuickAssessmentTitle
	                                    ? "Your original score and performance numbers remain unchanged."
                          ...
                          <Box
                            ...
                            sx={{
                              ...
                              '& a[href*="reattempt-subtopic-"]': {
                                color: '#ff6b35',
                                fontWeight: 'bold',
                                textDecorationColor: 'rgba(255, 107, 53, 0.45)',
                                cursor: 'pointer'
                              }
                            }}
